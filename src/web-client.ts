import { Call, Entry, convertCallStringToCallObj, CallType, encrypt, isAdmin } from "./utils/database";

/**
 * Loads entries from the server and renders them in the table.
 */
async function loadEntries() {
    const userLoggedInData = fetch('/user').then((res: Response) => res.json());
...
socket.addEventListener('message', function (event) {
    ...
    // Update the UI with the new entries
    loadEntries();
});
...
function renderEntries(entries: Map<string, Entry>, loggedIn: boolean, username: string | undefined) {
    ...
    Object.entries(entries).forEach(([uuid, entry]: [string, Entry]) => {
        ...
        // Modify action buttons based on user authentication
        ...
        if (!loggedIn || username === undefined) {
            // If user is not logged in, hide all action buttons
            editButton.style.display = 'none';
            deleteButton.style.display = 'none';
            wagerButton.style.display = 'none';
        } else {
            // If timeArrived is filled, then the bet has already took place.
        ...
        wagerButton.addEventListener('click', () => {
            ...
            // Populate the form fields based on the specific entry
            ...
            // Populate the form fields with entry details
